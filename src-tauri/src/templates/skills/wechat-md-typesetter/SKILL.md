...
## 第二段 · 调壹伴脚本：两段直送草稿（v5）
脚本随包落盘在 `~/PolarisGEO/skills/wechat-md-typesetter/scripts/wechat_yiban.py`。
...
3. **直送草稿**（两段自动连跑）：
   ...
   python ~/PolarisGEO/skills/wechat-md-typesetter/scripts/wechat_yiban.py \
     ...
     --title-file "<标题.txt 绝对路径>"
   ```
   - **中文标题一律走 `--title-file`**（UTF-8 文件）：Windows 控制台常把中文命令行参数传花，
     `--title "标题"` 只在纯 ASCII 时可靠。主题同理，中文传参不稳可用英文别名
     （moyun/minimal/tech/magazine/green/orange/paper/indigo）。
   - **浏览器与登录态前置**：优先 CDP 接管本机已开的调试端口 Chrome（9222，脚本结束不关窗，
     留给用户核对草稿）；没有就自起持久化上下文，登录态在 `~/.polaris-mp-profile`
     （可用 `POLARIS_MP_PROFILE` 覆盖），未登录会等扫码。
   - 注意：留窗核对期间该 profile 被占用，其他要用同一 profile 起浏览器的工具
     （如 post-to-wechat 的 mp_draft.py）会启动失败，先关核对窗再跑。
...
3.5 **补封面**（草稿已存、只差封面时）：
   ```bash
   python ~/PolarisGEO/skills/wechat-md-typesetter/scripts/wechat_yiban.py \
     --mode set-cover --cover "<封面图绝对路径>"
   ```
   接管已打开的草稿编辑器（没开会提示用户点进草稿），走图片库上传→选中→裁剪→保存，
   正文与标题一概不动。publish 模式则直接带 `--cover` 一步到位。
...
7. **可视化排版面板**（壹伴插件形态，用户要"自己看着改/挑模板/AI 改风格"时用这个）：
   ```bash
   python ~/PolarisGEO/skills/wechat-md-typesetter/scripts/wechat_yiban.py --mode panel
   ```
