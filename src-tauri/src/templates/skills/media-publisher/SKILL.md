...
| 知乎 | zhihu | **full+题图** | zhuanlan.zhihu.com/write（Draft.js）自动填+自动存草稿。**题图专用流程（2026-07-20 真机上线）**：`cover.direct_input`——必须先点「添加封面」label 激活封面组件（`reveal_click`），React 才认后续 `set_input_files`（喂 `input.UploadPicture-input` 真实文件走原生上传）；**注意合成 dataURL 粘贴对知乎 Draft.js 无效，绝不能靠塞正文首图当封面**。上传后回读 `img[alt='封面图']`（src=pic-private.zhihu.com…article_draft_web）确认落地。存草稿走 URL 跳 `/p/{id}/edit` 即成功（回执文案常抓不到，属正常） |
...
**封面/图片差异化**：`--images` 第一张按平台走对的通道——
- `cover.direct_input`（**知乎**）：先 `reveal_click` 点「添加封面」激活 → 喂原生 `input[type=file]` → 回读 `img[alt='封面图']` 确认；
- `cover`（**百家号**）：hover 缩略图出「更换」→ 弹窗本地上传 → 点确定；
- `image_upload`（**抖音**）：`file_chooser` 图库上传，首图默认封面；
- 其余（**头条**）：塞正文首图，平台自动采用为封面。

**封面是硬性产物，不是"发了就算"**：脚本对有专用封面流程的平台（知乎/百家）会**回读预览缩略图**，日志 `cover_set ok`
且 note 含「落地确认」才算成功。若 `cover_set ok=false`（喂入但没回读到预览），**必须重试一次或明确向用户报告"封面未确认落地，请到窗口核对/手动拖入"，不得默默当成功**。收尾报告里封面状态要单独讲清（已确认 / 未确认需手检）。
...
## 机动化数据爬虫 metrics_crawler.py（抓自己账号后台数据）

把**你自己在各平台创作者后台的数据**（阅读/点赞/评论/粉丝/播放/收益……）自动抓下来落盘，
...
（`source=metrics_crawler`、`kind=account_metrics`，与投递回执同库，数据复盘官直接能读）。
另存人类可读快照 `~/PolarisGEO/data/metrics_latest/{platform}.json` 方便自己直接打开看，
内含通用「指标速览」——脚本自动从任意 JSON 里挖出键名像阅读/点赞/粉丝/收益的数字（中英文皆识）。

已配置平台：头条 / 百家号 / 抖音 / 知乎 / B站 / 小红书（各 2 个数据页）、公众号（首页，数字多为
服务端渲染→主要靠万能口补）。**前置**：目标平台先在「账号中心」扫过码（未登录会报 `need_login` 并跳过，
不中断其它平台）。

```bash
python ~/PolarisGEO/skills/media-publisher/scripts/metrics_crawler.py --list          # 看能抓哪些平台/数据页
python ~/PolarisGEO/skills/media-publisher/scripts/metrics_crawler.py --platform toutiao
...
4. 收尾报告：平台、投递结果（草稿已存/需手贴哪些）、**封面落位状态（已确认落地 / 未确认需手检——见 `cover_set` 回执）**、其余配图情况、
   一句「请到平台后台草稿箱核对（重点看封面），确认后自行发布」。封面未确认时要把话说明白，别让用户以为已就位。
